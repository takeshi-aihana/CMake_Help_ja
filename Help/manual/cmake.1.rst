...
ソースツリー（Source Tree）
  プロジェクトによって提供されたソースファイルなどが配置されているプロジェクト最上位のディレクトリには、マニュアルの :manual:`cmake-language(7)` に記載されている ``CMakeLists.txt`` というファイルを配置し、その中にビルドシステムを指定します。
  このファイルは複数のサブディレクトリに配置でき、その場合は :manual:`cmake-buildsystem(7)` で説明されているように、ビルド・ターゲットやそのれらの依存関係をそれぞれ指定します。

ビルドツリー（Build Tree）
  プロジェクト最上位のディレクトリには、生成されたビルドシステムのファイルと、それを使ってビルドした結果（たとえば、実行形式やライブラリ）が格納されます。
...
.. option:: --system-information [file]
 ...
 CMake プロジェクトのビルドツリーの最上位で実行すると、キャッシュやログファイルなどの追加情報もダンプする。
...
.. option:: -v, --verbose
  ...
  環境変数 :envvar:`VERBOSE` またはキャッシュ変数 :variable:`CMAKE_VERBOSE_MAKEFILE` がセットされている場合、このオプションを省略できる。
