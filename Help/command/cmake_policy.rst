...
CMake は任意のスタック上にポリシーの設定を保存しているので、``cmake_policy`` コマンドによる変更はスタックのトップにのみ影響します。
...
スタック上のエントリを削除するには、PUSH 操作に対応する POP 操作が必要です。
そして、これはポリシーを「一時的に」変更したい場面で便利なインタフェースです。
