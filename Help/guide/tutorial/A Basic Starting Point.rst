...
``CMakeLists.txt`` から :command:`configure_file` コマンドを呼び出すと、``@Tutorial_VERSION_MAJOR@`` と ``@Tutorial_VERSION_MINOR@`` が対応するプロジェクトのバージョン番号に置き換えられて、出力用ヘッダファイル ``TutorialConfig.h`` に書き出されます。
