...
この関数は、実際にソース・コードをコンパイルする前に、必要となる依存を確認する簡単なテスト・コードを作成してコンパイルします。
...
``log`` と ``exp`` の両方の関数が利用できるならば :command:`target_compile_definitions` コマンドを使って、``HAVE_LOG`` と ``HAVE_EXP`` をコンパイル時の ``PRIVATE`` な定義フラグとして指定できるようにします。
