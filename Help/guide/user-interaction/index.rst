...
これらのツールは、キャッシュ・エントリを編集した後に必要に応じてソフトウェアを再構成したり、ビルドシステムを再生成するための対話型インタフェースを提供しています。
このツールの中では、キャッシュ・エントリに関連付けられた短い説明文（ヘルプ）も表示される場合があります。

また、それぞれのキャッシュ・エントリは型を持ち、その型に応じでツールの中で見え方や操作が異なる場合があります。
たとえば ``BOOL`` 型のキャッシュ・エントリはチェックボックスのユーザ・インタフェースで編集でき、``STRING`` 型はテキストボックス、``FILEPATH`` 型は ``STRING`` と同様にファイルシステム上のパスを特定するための手段としてファイル・ダイアログを提供しています。
:manual:`cmake-gui(1)` ツールの場合、``STRING`` 型のキャッシュ・エントリには、選択可能な文字列が一覧になったドロップ・ダウン形式のリストが提供される場合があります
...
このコマンドは、パラメータを説明するヘルプとデフォルト値を含むキャッシュ・エントリを生成します。
通常、このようなキャッシュ・エントリはソフトウェアに固有の情報であり、ビルド時にテストやサンプルをビルドするかどうか、あるいは例外を有効にしてビルドするかどうか等の設定に影響するものです。
...
これらのプリセットにはビルド・ディレクトリやジェネレータ、キャッシュ・エントリ、環境変数、およびその他のコマンドライン・オプションをセットできます。
...
その中からプリセットを選択するとバイナリ・ディレクトリ、ジェネレータ、環境変数、そしてキャッシュ・エントリがセットされますが、これらのすべてはプリセットを選択したあとで上書きで再設定することも可能です。
